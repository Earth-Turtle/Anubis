...
class Responses:
    ...
    def find_response(self, message: Message) -> str:
        ...
        if message.author.name.lower() == "biological_jessie" and re.search(":3", message.content) and self.timer(":3", 10):
            return "https://cdn.discordapp.com/attachments/525173455516336129/1135766326233796608/rapidsave.com_meow-fujthvqougeb1.mp4"
        if re.search(r"\bpegging\b", message.content.lower()) and self.timer("pegging", 10):
            return "https://imgflip.com/i/7uygbp"
        if re.search(r"\bfish\b", message.content) and self.timer("fish", 10):
